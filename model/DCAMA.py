...
class DCAMA(nn.Module):
    ...
    def extract_feats(self, img):
        ...
        feat = self.feature_extractor.act1.forward(feat)
        ...
        for hid, (bid, lid) in enumerate(zip(bottleneck_ids, self.lids)):
            ...
            feat = self.feature_extractor.__getattr__("layer%d" % lid)[bid].act1.forward(feat)
            ...
            feat = self.feature_extractor.__getattr__("layer%d" % lid)[bid].act2.forward(feat)
            ...
            feat = self.feature_extractor.__getattr__("layer%d" % lid)[bid].act3.forward(feat)
    ...
    def forward(self, query_feats, support_feats, support_mask, nshot=1):
        ...
        for idx, query_feat in enumerate(query_feats):
            ...
            if nshot == 1:
                ...
                mask = F.interpolate(
                    support_mask.unsqueeze(1).float(), support_feat.size()[2:], mode="bilinear", align_corners=True
                ).view(support_feat.size()[0], -1)
                support_feat = (
                    support_feat.view(support_feat.size()[0], support_feat.size()[1], -1).permute(0, 2, 1).contiguous()
                )
            else:
                ...
                mask = torch.stack(
                    [
                        F.interpolate(k.unsqueeze(1).float(), (ha, wa), mode="bilinear", align_corners=True)
        ...
        coarse_masks1 = (
            torch.stack(coarse_masks[self.stack_ids[2] - self.stack_ids[0] : self.stack_ids[3] - self.stack_ids[0]])
            .transpose(0, 1)
            .contiguous()
            .view(bsz, -1, ha, wa)
        )
        ...
        coarse_masks2 = (
            torch.stack(coarse_masks[self.stack_ids[1] - self.stack_ids[0] : self.stack_ids[2] - self.stack_ids[0]])
            .transpose(0, 1)
            .contiguous()
            .view(bsz, -1, ha, wa)
        )
        ...
        if nshot == 1:
            ...
            support_feat = (
                torch.stack([support_feats[k][self.stack_ids[1] - 1] for k in range(nshot)]).max(dim=0).values
            )
        ...
        if nshot == 1:
            ...
            support_feat = (
                torch.stack([support_feats[k][self.stack_ids[0] - 1] for k in range(nshot)]).max(dim=0).values
            )
    ...
    def build_conv_block(self, in_channel, out_channels, kernel_sizes, spt_strides, group=4):
        ...
        for idx, (outch, ksz, stride) in enumerate(zip(out_channels, kernel_sizes, spt_strides)):
            ...
            building_block_layers.append(
                nn.Conv2d(in_channels=inch, out_channels=outch, kernel_size=ksz, stride=stride, padding=pad)
            )
