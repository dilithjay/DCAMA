...
class DatasetSerpMini(Dataset):
    ...
    def build_img_metadata_classwise(self):
        train_imgs = sorted(glob(os.path.join(self.base_path, self.split_coco, "*.npy")))
        print("Len", len(train_imgs))
        if self.split_coco == "val":
            train_imgs += sorted(glob(os.path.join(self.base_path, "train", "*.npy")))
            print("Len v", len(train_imgs))
        img_metadata_classwise = {
            0: list(map(lambda x: os.path.relpath(x, os.path.dirname(os.path.dirname(x))), train_imgs))
        }
        return img_metadata_classwise
    ...
    def load_frame(self):
        class_sample = np.random.choice(self.class_ids, 1, replace=False)[0]
        query_name = self.img_metadata_classwise[class_sample][0]
        query_img = np.load(os.path.join(self.base_path, query_name)).astype(float).transpose(1, 2, 0)
        query_mask = self.read_mask(query_name)
        ...
        support_names = self.img_metadata_classwise[class_sample][1 : 1 + self.shot]
        support_imgs = []
        support_masks = []
        for support_name in support_names:
            support_img = np.load(os.path.join(self.base_path, support_name)).astype(float).transpose(1, 2, 0)
